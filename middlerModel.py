...
class VideoSegmentationNetwork(nn.Module):
    ...
    def __init__(self):
        ...
        self.transenc = Transformer_Encoder(input_dim=512, num_layers=4, num_heads=4, dropout=0.1)
    ...
    def __get_positional__tensor(self, embedding_dim=EMBEDDED_DIMENSION):
        pos_tensor1 = self.get_positional_encoding(seq_len=1024, embedding_dim=16)
        pos_tensor2 = self.get_positional_encoding(seq_len=2, embedding_dim=16)
        # pos_embedding = nn.Parameter(torch.randn(SEQUENCE_LENGTH, EMBEDDED_DIMENSION, requires_grad=True, device=DEVICE))
        pos_tensor1 = torch.cat([pos_tensor1, pos_tensor1], dim=0)
        pos_tensor2 = torch.cat([pos_tensor2[:, i].repeat_interleave(1024).unsqueeze(1) for i in range(pos_tensor2.shape[1])], dim=1)
