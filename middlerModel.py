...
class VideoSegmentationNetwork(nn.Module):
    ...
    def __get_positional__tensor(self, embedding_dim=EMBEDDED_DIMENSION):
        ...
        pos_tensor1 = torch.cat([pos_tensor1[i].repeat(CHUNK_LENGTH, 1) for i in range(pos_tensor2.shape[0])], dim=0)
        pos_tensor2 = torch.cat([pos_tensor2[i].repeat(CHUNK_LENGTH, 1) for i in range(pos_tensor1.shape[0])], dim=0)
